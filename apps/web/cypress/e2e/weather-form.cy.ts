describe("Weather Form Functionality", () => {
	...
	it("should update days display when slider is moved", () => {
		...
		cy.get('[data-testid="days-slider"]').then(($slider) => {
			...
			cy.get('[data-testid="days-slider"]').click(
				x - sliderRect.left,
				y - sliderRect.top,
			);
		});
	...
	it("should display temperature results for a valid location with default days", () => {
		...
		// Enter a location
		cy.get('[data-testid="location-input"]').type("San Francisco");
	...
	it("should display error message for an invalid location", () => {
		...
		cy.get('[data-testid="location-input"]').type("NonExistentPlace123");
	...
	it("should submit form with custom days value and show correct data", () => {
		...
		cy.intercept("GET", /\/weather\/average\?.*/, (req) => {
			...
			const mockResponse = {
				...
				dailyTemperatures: Array(parseInt(daysParam || "7"))
					.fill(null)
					.map((_, i) => ({
						date: `2025-04-${25 + i}`,
		...
		// Enter a location
		cy.get('[data-testid="location-input"]').type("Tokyo");
		...
		cy.get('[data-testid="date-range"]').should(
			"contain",
			"Range: 2025-04-25 - 2025-05-24",
		);
	...
	it("should disable submit button when location is empty", () => {
		...
		cy.get('[data-testid="location-input"]').type("Berlin");
