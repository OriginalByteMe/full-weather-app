...
export default function Home() {
	...
	const [overallAverageTemperature, setOverallAverageTemperature] = useState<
		number | null
	>(null);
	const [dailyChartData, setDailyChartData] = useState<
		Array<{ date: string; temperature: number | null }>
	>([]);
	const [fetchedLocationName, setFetchedLocationName] = useState<string | null>(
		null,
	);
	...
	const fetchWeatherData = async (
		...
		const response = await fetch(
			`http://localhost:3000/weather/average?city=${encodeURIComponent(location)}&days=${days}`,
		);
		...
		if (!response.ok) {
			...
			try {
				...
				throw new Error(
					`Request failed with status ${response.status}. Could not parse error response.`,
				);
			}
			throw new Error(
				errorData?.error ||
						...
						<div className="grid gap-2">
							<Label htmlFor="days">
								...
								{weatherMutation.data &&
									weatherMutation.data.startDate &&
									weatherMutation.data.endDate && (
										<span
											data-testid="date-range"
											className="text-sm text-muted-foreground ml-2"
										>
											(Range: {weatherMutation.data.startDate} -{" "}
											{weatherMutation.data.endDate})
										</span>
									)}
							</Label>
					...
					<CardFooter className="flex flex-col gap-4">
						...
						{weatherMutation.isPending && (
							<div
								...
								className="mt-4 w-full rounded-md border bg-muted p-4 text-center"
							>
						...
						{weatherError &&
							...
							!weatherMutation.isSuccess && (
								<div
									...
									className="mt-4 w-full rounded-md border border-red-500 bg-destructive/10 p-3 text-center"
								>
							...
							!weatherError && (
								<Card className="mt-4 w-full">
									<CardHeader>
										...
										<CardDescription>
											Historical daily average temperatures for{" "}
											{fetchedLocationName}.
										</CardDescription>
									</CardHeader>
									<CardContent className="grid gap-6">
										{overallAverageTemperature !== null && (
											<div
												data-testid="temperature-result"
												className="text-center mb-4"
											>
												<p className="text-2xl font-bold">
													Overall Average: {overallAverageTemperature}°C
												</p>
												<p className="text-xs text-muted-foreground">
													For period: {weatherMutation.data?.startDate} to{" "}
													{weatherMutation.data?.endDate} (
													{weatherMutation.data?.daysFetched} days)
												</p>
											</div>
										)}
